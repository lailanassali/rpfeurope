...
export function MenuItem({
	...
	React.useEffect(() => {
		if (isDrawer && hasActiveChild) {
			setIsOpen(true);
		}
	}, [isDrawer, hasActiveChild]);
			...
			return (
				<div className="">
					<button
						onClick={handleClick}
						className={cn(
							"flex items-center gap-[11px] w-full pb-[12px] transition-colors hover:text-primary",
							hasActiveChild && "text-primary",
							className,
						)}
					>
						{icon && <span className="shrink-0 text-[#000000B2]">{icon}</span>}
						<span
							className={cn(
								"text-[14px] text-black",
								hasActiveChild ? "font-bold text-primary" : "font-medium",
							)}
						>
							{title}
						</span>
					...
					{isOpen && (
						<div className="pl-[28px] pb-[8px] space-y-2">
	...
	if (icon && isDrawer) {
		return (
			...
			>
				<div className="shrink-0 text-[#000000B2]">{icon}</div>
